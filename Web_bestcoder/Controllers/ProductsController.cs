...
namespace Web_bestcoder.Controllers
{
    public class ProductsController : Controller
    {
        ...
        // Method to save products as a JSON file
        public IActionResult SaveProductsToJson()
        {
            string filePath = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot", "products.json");
            string jsonString = JsonSerializer.Serialize(products);
            System.IO.File.WriteAllText(filePath, jsonString);
            return Content("Products have been saved to JSON.");
        }
    }

    // Product model (Assuming you have a model similar to this)
